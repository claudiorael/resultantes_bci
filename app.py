...
if file:
    try:
        ...
        if st.button("🚀 PROCESAR"):
            ...
            status.markdown("**90%** - Ordenando reporte y generando gráfico de ventas...")
            ...
            # --- GRÁFICO DE VENTAS POR CAMPAÑA ---
            st.markdown("---")
            st.markdown("### 📊 Resumen de Ventas por Campaña")
            
            # Filtramos solo los registros que son "VENTA"
            df_ventas = res[res['GES_descripcion_3'].str.contains('VENTA', na=False)]
            
            if not df_ventas.empty:
                # Agrupamos por campaña y contamos
                conteo_ventas = df_ventas.groupby('GES_nombre_campana_gestion').size().reset_index(name='Ventas')
                conteo_ventas = conteo_ventas.set_index('GES_nombre_campana_gestion')
                
                # Mostramos el gráfico de barras nativo de Streamlit en naranja
                st.bar_chart(conteo_ventas, color="#FF7F00")
            else:
                st.info("No se registraron ventas ('VENTA' en la tipificación) en este reporte.")
