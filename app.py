...
col_cfg, col_up = st.columns([1, 2.5])
with col_cfg:
...
if file:
    try:
        ...
        if st.button("🚀 PROCESAR"):
            ...
            res = res.reindex(columns=cols).astype(str).apply(lambda x: x.str.upper()).replace(['NAN', 'NONE', '<NAT>', '<NA>'], '')
            ...
            # --- PANEL DE GRÁFICOS ---
            st.markdown("---")
            st.markdown("### 📊 Panel de Desempeño Operativo")
            ...
            if not dv.empty:
                ...
                with c1:
                    ...
                    ch1 = alt.Chart(cv).encode(x=alt.X('GES_nombre_campana_gestion:N', title='Campaña'))
                ...
                with c2:
                    ...
                    dv_h = dv.copy()
                    dv_h['H'] = dv_h['GES_hora_min_creacion'].str.split(':').str[0].replace('', '0').astype(int)
                    ch = dv_h.groupby('H').size().reset_index(name='V').sort_values('H')
                    ch2 = alt.Chart(ch).encode(x=alt.X('H:O', title='Hora (H)'))
