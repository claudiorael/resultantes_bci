        ...
        if st.button("🚀 PROCESAR"):
            ...
            if not df_ventas.empty:
                ...
                with col_chart1:
                    ...
                    # Gráfico Altair Profesional con Labels (CORREGIDO)
                    base = alt.Chart(conteo_ventas).encode(
                        x=alt.X('GES_nombre_campana_gestion:N', title='Campaña')
                    )
                    
                    # Barras naranjas acortadas
                    barras = base.mark_bar(color="#FF9800", cornerRadiusTopLeft=4, cornerRadiusTopRight=4).encode(
                ...
                with col_chart2:
                    ...
                    # Gráfico Altair Profesional para horas (CORREGIDO)
                    base_hora = alt.Chart(conteo_horas).encode(
                        x=alt.X('Hora:O', title='Hora (H)') 
                    )
                    
                    # Barras azules acortadas
                    barras_hora = base_hora.mark_bar(color="#003366", cornerRadiusTopLeft=4, cornerRadiusTopRight=4).encode(
